...
namespace bizx.views.serviceDeskManager
{
    ...
    public partial class SalaryAdvanceApprovalViewPage : ContentPage
    {
        int mServiceReqId = 0;
        int flag = 0;
        ...
        public SalaryAdvanceApprovalViewPage(int ServiceReqId, bool isManagerLogin, string catalogueName, int id)
        {
            InitializeComponent();
        ...
        private async Task<ServiceRequestMasterDetailModel> InitApiCalling()
        {
            ...
            if (mServiceRequestDetail != null && mServiceRequestDetail.authenticated)
            {
                ...
                await GetSalaryDetails(mServiceRequestMasterDetail);
        ...
        private async Task<ServiceRequestMasterDetailModel> GetSalaryDetails(ServiceRequestMasterDetailModel apiResult)
        {
            var GetSalaryDetailsResponse = await App.RestService.GetResponse<SalaryAdvanceModel>
                (Constants.URL + "ServiceManagement/ServiceManagementMasterSalaryAdvanceDetailsById?ServiceManagementMasterId="
                + Util.Encode(apiResult.ServiceRequestDetail.id.ToString()));


            if (GetSalaryDetailsResponse != null && GetSalaryDetailsResponse.authenticated)
            {
                mServiceRequestMasterDetail.SalaryAdvanceDetailModel = GetSalaryDetailsResponse.data;
                mServiceRequestMasterDetail.SalaryAdvance = GetSalaryDetailsResponse.data.srmasterdetails[0];
                mServiceRequestMasterDetail.ServiceRequestDetail.parentCategoryName = "Advance Salary Request";
                par.Text= "Advance Salary Request";
                if (mServiceRequestMasterDetail.SalaryAdvance.attachmentFileName.Equals(""))
                {
        ...
        {
            DownloadFile(mServiceRequestMasterDetail.SalaryAdvance.attachmentFileName, mServiceRequestMasterDetail.SalaryAdvance.attachment, Constants.URL + "ServiceManagement/" +
                "DownloadServiceAttachmentByFilename?Filename=" + mServiceRequestMasterDetail.SalaryAdvance.attachment + "&ServiceManagementMasterId=" +
                Util.Encode(Convert.ToString(mServiceRequestMasterDetail.SalaryAdvance.id)));
        }
